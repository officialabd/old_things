...
public class GameLogic {
    ...
    public void update() {
        ...
        boolean outOfBounds = snake.getX() > map.getSizeX() - 1 || snake.getX() < 0 || snake.getY() > map.getSizeY() - 1 || snake.getY() < 0;
        if (collided || outOfBounds) {
            snake.reset();
            score = 0;
        }
        if (slowerTimer != null && System.currentTimeMillis() - slowerTimer.getSlowModeTime() >= 10000 && slowerTimer.isTaken()) {
            snake.setSpeed();
            slowerTimer = null;
        }
        checkSlowerTimer();
